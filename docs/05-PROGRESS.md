...
### Terceira varredura completa (3 agentes em paralelo: estrutural, segurança round 2, layout/design) + achado crítico de XSS

O usuário pediu uma varredura completa, repetida "umas três vezes", cobrindo estrutura, segurança e design. Antes de rodar os agentes, verifiquei manualmente um risco sistêmico que eu mesmo tinha descoberto ao corrigir `confirm_upload`: `admin_conn` nunca tem `auth.uid()` funcional (claims não são setados nessa conexão). Gregei todos os ~20 outros usos de `auth.uid()` no backend e confirmei que **todos os outros** rodam corretamente em `conn` (RLS) — o bug era mesmo isolado ao `confirm_upload`, já corrigido. Nenhuma ação adicional necessária aqui.

Depois, 3 agentes especializados rodaram em paralelo. Verifiquei manualmente os achados mais graves de cada um antes de agir (mesmo processo das rodadas anteriores).

**Corrigidos nesta rodada:**

- **[CRÍTICO] XSS armazenado no snippet de busca** (`Search.tsx`): o snippet vem de `ts_headline()` do Postgres sobre o `ocr_text` do documento — texto controlado por quem faz upload (qualquer editor+ da empresa) — e era injetado direto via `dangerouslySetInnerHTML`, só trocando a tag `<mark>` por uma versão com classe CSS, sem escapar o resto. Um documento com OCR contendo algo como `<img src=x onerror=fetch(...+localStorage.docke_token)>` executaria esse script pra qualquer usuário que buscasse um termo que desse match nesse documento — e como o token de sessão fica em `localStorage`, isso é uma cadeia completa de roubo de sessão dentro da mesma empresa. Corrigido: o snippet inteiro agora passa por `element.textContent` (que escapa tudo) antes de reconverter só os marcadores de `<mark>` de volta pra tags reais. Testado isoladamente com um payload malicioso real — confirmado que o `<img>` vira texto inofensivo e o destaque `<mark>` continua funcionando.
- **DoS no modo demo via chave de rate-limit compartilhada**: `DEMO_EMAIL` é público (aparece no código do frontend). Um atacante não-autenticado podia mandar 6 senhas erradas pra `demo@docke.app` via `/auth/login` e travar `/auth/demo-login` — usado por qualquer usuário real querendo entrar no modo demo — por 15 minutos, já que os dois endpoints compartilhavam a mesma chave de bloqueio por e-mail. Corrigido: o modo demo agora usa uma chave de rate-limit isolada, independente do bucket do e-mail público. Testado isoladamente: mesmo com o bucket do e-mail público bloqueado, o bucket que o demo-login realmente usa continua livre.
- **`ShareModal.tsx` revogava link sem nenhuma confirmação**: diferente da nova tela "Compartilhados" (que já usa `ConfirmModal`), o modal de compartilhar de dentro do painel de detalhes do documento apagava o link no primeiro clique, sem aviso — violando a própria regra do design system (revogar = tier "Médio", exige `ConfirmModal` com variante danger). Corrigido pra usar o mesmo padrão. Testado ao vivo: criado um link real, clicado em revogar, modal de confirmação apareceu corretamente por cima do modal de compartilhar (sem vazamento visual), confirmado, link sumiu da lista.
- **Dashboard "Documentos recentes" não navegava ao clicar**: o backend já retornava `folder_id` (usado em outro lugar), só faltava o `onClick` no item da lista — único caso, entre vários pontos parecidos revisados pelo agente estrutural, onde o dado já existia ponta a ponta e só faltava a interação. Corrigido e testado ao vivo (clique abriu o documento certo com o Detail Drawer correto).
- **`CompanyContext` não sincronizava `permission_level` quando a empresa em cache continuava válida**: a correção anterior desta sessão (revalidar contra `/companies`) só substituía a empresa em cache quando o `id` não existia mais — se o `id` continuava válido, os outros campos (como `permission_level`) ficavam presos no valor antigo. Cenário real: um admin rebaixado por outro admin enquanto a sessão dele continua aberta seguiria vendo os controles de Configurações de admin até trocar de empresa manualmente. Corrigido: agora sempre sincroniza o objeto inteiro com o dado fresco do servidor quando o id ainda é válido.
- **Empty state de "Compartilhados" sem call-to-action**: adicionado o mesmo botão "Ir para Documentos" que "Ancorados" já tinha, por consistência (achado de baixo custo do agente de design).

**Investigado e descartado (achados que pareciam graves mas não são, após verificação):**
- `GET /shares` com filtro de `company_id` + os novos LEFT JOINs — não vaza dado entre empresas: os LEFT JOINs rodam em `conn` (RLS), então mesmo num JOIN "solto" por `resource_id`, a RLS de `documents`/`folders` já filtra por acesso real do usuário; `company_id` de um documento não é transferível entre empresas no código atual.
- Lógica de bloqueio do rate-limit (ordem de checagem, isenção de tentativa bem-sucedida) — lida com cuidado, sem bug.
- Ramo de restauração de **pasta** em `POST /trash/:id/restore` (só o ramo de documento tinha sido conferido antes) — mesma estrutura segura do ramo de documento, permissão sempre checada contra o `company_id`/`path` reais do banco, nunca fornecidos pelo cliente.
- Tratamento visual "vidro" do `Shares.tsx` — o agente de design inicialmente suspeitou de inconsistência, mas confirmou (lendo `docs/06-DESIGN-SYSTEM.md`) que o "vidro" é intencionalmente restrito a painéis estruturais (Sidebar, Topbar, tabela, stat cards) — `Shares.tsx` usa corretamente o mesmo padrão "cartão simples" que `Favorites.tsx`/`Trash.tsx`/`Activity.tsx` já usam. Não é uma inconsistência.
